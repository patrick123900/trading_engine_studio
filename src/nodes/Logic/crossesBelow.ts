...
const crossesBelowNode: NodeModule = {
  definition: {
    type: "logic.crossesBelow",
    title: "Crosses Below",
    description: "Return true when the left series crosses below the right series.",
    color: "#3182ce",
    inputs: [
  ...
  executor: {
    ...
    run: ({ inputs }) => {
      ...
      if (!left || !right) {
        throw new Error("Crosses Below requires two numeric inputs.");
      }
...
export default crossesBelowNode;
