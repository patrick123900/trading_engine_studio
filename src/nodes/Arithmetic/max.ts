...
const maxNode: NodeModule = {
  definition: {
    type: "arithmetic.max",
    title: "Max",
    description: "Return the larger of two numeric inputs.",
    color: "#f59e0b",
    inputs: [
  ...
  executor: {
    ...
    run: ({ inputs }) => {
      ...
      if (!left || !right) {
        throw new Error("Max requires two numeric inputs.");
      }

      const length = Math.max(left.length, right.length);
      return {
        values: Array.from({ length }, (_, index) => Math.max(left[Math.min(index, left.length - 1)], right[Math.min(index, right.length - 1)])),
      };
    },
  },
};
